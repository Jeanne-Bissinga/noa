...
// Accès du collaborateur à ses préférences de travail, sans compte Noa.
//
// Le collaborateur n'est pas un utilisateur du produit : Noa reste une
// plateforme de recruteurs et de managers. Il n'intervient qu'une fois, pour
// renseigner ses préférences de travail avant son arrivée — un lien porteur
// d'un token suffit donc, sans seconde authentification ni portail.
//
// ─── Le token brut n'est jamais persisté ────────────────────────────────────
// Seule son empreinte SHA-256 est stockée (token_hash sur la table concernée,
// cf. scripts/011 et 012). Le token en clair n'existe qu'à deux instants :
// dans la réponse de l'action qui l'émet, et dans l'URL que le collaborateur
// ouvre. Un accès en lecture à la base ne donne accès à aucune page.
//
...
// L'empreinte est calculée sur `purpose:token`. Un token émis pour un point de
// suivi ne peut donc jamais résoudre une page de préférences, même si son
...
// La RLS ne donne accès à ces tables qu'au recruteur de l'entreprise. Le
...
export async function resolveWorkPreferencesToken(token: string): Promise<WorkPreferencesResolution> {
  ...
  const { data, error } = await supabase
    ...
    .select("id, status, token_hash, token_expires_at, questionnaire_answers, onboarding:onboardings(candidate:candidates(first_name))")
  ...
  return {
    ...
    preferences: {
      ...
      firstName: onboarding?.candidate?.first_name ?? "",
