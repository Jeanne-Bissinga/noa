...
/** Première question sans réponse : point de reprise. null quand tout est répondu. */
export function firstUnansweredIndex(answers: Record<string, number>): number | null {
  const index = WORK_PREFERENCE_QUESTIONS.findIndex((q) => !isAnswerValue(answers[q.id]));
